...
import { CheckCircle, Upload, FileText, CreditCard, Building2, ChevronRight, ChevronLeft, Eye, X, AlertCircle, Clock, Shield } from "lucide-react";
...
const S = {
  ...
  stepCircle: (active, done) => ({
    ...
    justifyContent: "center", fontWeight: 700, fontSize: 15, zIndex: 1,
  ...
  stepLine: (done) => ({
    flex: 1, height: 3, background: done ? "#22c55e" : "#e2e8f0",
    marginBottom: 22, transition: "all .3s",
  }),
  /* card */
  card: {
    background: "#fff", borderRadius: 18, boxShadow: "0 4px 24px rgba(0,0,0,.07)",
  ...
  filePreviewRow: {
    ...
    background: "#f1f5f9", borderRadius: 10, marginTop: 8,
  },
  filePreviewName: { fontSize: 13, flex: 1, fontWeight: 500, color: "#1e293b", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" },
  /* buttons */
  btnPrimary: {
  ...
  btnRow: { display: "flex", justifyContent: "flex-end", gap: 12, marginTop: 36 },
  /* convention */
  conventionBox: {
    border: "1.5px solid #e2e8f0", borderRadius: 12, padding: "24px 28px",
    maxHeight: 340, overflowY: "auto", fontSize: 13, lineHeight: 1.75,
    color: "#374151", background: "#fafafa",
  },
  checkRow: {
    display: "flex", alignItems: "flex-start", gap: 12, marginTop: 24,
    padding: "16px 20px", background: "#eff6ff", borderRadius: 10,
    border: "1.5px solid #bfdbfe",
  },
  /* plans */
  plansGrid: { display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 20, marginTop: 8 },
  planCard: (selected, color) => ({
    border: selected ? `2.5px solid ${color}` : "2px solid #e2e8f0",
    borderRadius: 14, padding: "24px 20px", cursor: "pointer",
    background: selected ? `${color}08` : "#fff", transition: "all .2s",
    position: "relative",
  }),
  planBadge: (color) => ({
    position: "absolute", top: -1, right: 16,
    background: color, color: "#fff", fontSize: 10, fontWeight: 700,
    padding: "2px 10px", borderRadius: "0 0 8px 8px", letterSpacing: ".04em",
  }),
  ...
  /* status */
  statusCard: (color, bg) => ({
    background: bg, border: `1.5px solid ${color}`, borderRadius: 14,
    padding: "20px 24px", display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 16,
  }),
};

/* ─── Delegations par gouvernorat ───────────────────────────────── */
const DELEGATIONS = {
...
const DOCS_CONFIG = [
  ...
  { key: "cin", label: "CIN du gérant (recto/verso)", hint: "PDF ou image, max 5 Mo", accept: ".pdf,.jpg,.jpeg,.png" },
  { key: "logo", label: "Logo de l'agence", hint: "PNG ou JPG recommandé, max 2 Mo", accept: ".png,.jpg,.jpeg" },
];
...
const PLANS = [
  {
    ...
    features: [
      ...
      { label: "Boosts offerts",               ok: false, note: "0 / mois" },
      { label: "Espace boutique agence",       ok: false },
      { label: "Logo + vidéo présentation",    ok: false },
      { label: "Badge Agence Certifiée",       ok: false },
      { label: "CRM centralisé",               ok: false },
      { label: "Leads qualifiés",              ok: false, note: "0" },
    ],
  },
  {
    id: "start", name: "Agency Start", price: 199, period: "/ mois",
    color: "#10b981", badge: "3 mois offerts", popular: false,
    features: [
      { label: "30 annonces actives",                  ok: true  },
      { label: "3 comptes agents inclus",              ok: true  },
      { label: "15 boosts / mois",                     ok: true  },
      { label: "150 Refresh / mois",                   ok: true  },
      ...
      { label: "Leads qualifiés",                      ok: false, note: "0 / mois" },
    ],
  },
  {
    id: "pro", name: "Agency Pro", price: 349, period: "/ mois",
    color: "#10b981", badge: "Recommandé", popular: true,
    features: [
      ...
      { label: "Alertes prospects",                    ok: true  },
      { label: "Support WhatsApp 24h/24 7j/7",         ok: true  },
      { label: "20 leads qualifiés / mois",            ok: true  },
    ],
  },
  {
    id: "power", name: "Agency Power", price: 499, period: "/ mois",
    color: "#065f46", badge: "Premium", popular: false,
    features: [
      ...
      { label: "Alertes prospects",                    ok: true  },
      { label: "Support WhatsApp 24h/24 7j/7",         ok: true  },
      { label: "50 leads qualifiés / mois",            ok: true  },
    ],
  },
];
...
Entre la société LOCALIZI.TN (ci-après « la Plateforme ») et l'Agence soussignée (ci-après « l'Agence »).

ARTICLE 1 — OBJET
La présente convention a pour objet de définir les modalités de collaboration entre la Plateforme et l'Agence dans le cadre de la diffusion d'annonces immobilières sur le portail LOCALIZI.TN.

ARTICLE 2 — ENGAGEMENTS DE L'AGENCE
L'Agence s'engage à :
  • Fournir des informations exactes et vérifiables sur son identité et son activité.
  • Publier uniquement des annonces conformes à la réglementation tunisienne en vigueur.
  • Ne pas diffuser de fausses annonces, d'annonces doublons, ou de contenus trompeurs.
  • Respecter les conditions générales d'utilisation de la Plateforme.
  • Mettre à jour ou supprimer les annonces dont les biens ne sont plus disponibles.
  • Traiter les prospects et clients avec professionnalisme et diligence.

ARTICLE 3 — ENGAGEMENTS DE LA PLATEFORME
La Plateforme s'engage à :
  • Mettre à disposition de l'Agence un espace dédié et sécurisé.
  • Assurer la visibilité des annonces conformément au plan d'abonnement souscrit.
  • Fournir les outils de gestion (tableau de bord, statistiques, boosts).
  • Traiter les données personnelles conformément à la réglementation tunisienne sur la protection des données.

ARTICLE 4 — DURÉE
La convention est conclue pour la durée du plan d'abonnement souscrit, renouvelable par tacite reconduction, sauf résiliation par l'une des parties avec un préavis de 30 jours.

ARTICLE 5 — CONFIDENTIALITÉ
Les parties s'engagent à maintenir la stricte confidentialité de toutes les informations échangées dans le cadre de la présente convention.

ARTICLE 6 — RÉSILIATION
La Plateforme se réserve le droit de résilier la convention en cas de non-respect des présents engagements, après mise en demeure restée sans effet pendant 7 jours.
...
En cochant la case ci-dessous, l'Agence déclare avoir lu, compris et accepté l'intégralité de la présente convention de partenariat.`;
...
function UploadField({ config, value, onChange, onPreview }) {
  ...
  return (
    <div>
      ...
      >
        <Upload size={20} color={drag ? "#2563eb" : "#94a3b8"} />
        ...
        <input
          ref={ref} type="file" accept={config.accept} style={{ display: "none" }}
      ...
      {value && (
        <div style={S.filePreviewRow}>
          ...
          <button
            ...
            style={{ background: "none", border: "none", cursor: "pointer", color: "#2563eb", display: "flex", alignItems: "center", gap: 4, fontSize: 12, fontWeight: 600 }}
          >
            <Eye size={14} /> Voir
          </button>
          <button
        ...
        <div style={{ flex: 1, overflow: "auto", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, background: "#f8fafc" }}>
          ...
          {!isImage && !isPDF && (
            <div style={{ textAlign: "center", color: "#64748b" }}>
              <FileText size={48} style={{ marginBottom: 12, opacity: .4 }} />
              <div style={{ fontSize: 14 }}>Aperçu non disponible pour ce type de fichier.</div>
...
const LEGAL_REQUIRED = [
  { key: "nom_agence",       label: "Raison sociale" },
...
/* ─── Steps ─────────────────────────────────────────────────────── */
function StepLegal({ data, onChange, showErrors }) {
  const errors = showErrors ? getLegalErrors(data) : {};
  const delegations = data.gouvernorat ? (DELEGATIONS[data.gouvernorat] || []) : [];
  ...
  const errMsg = (key) => errors[key]
    ? <span style={{ fontSize: 11.5, color: "#ef4444", marginTop: 2 }}>{errors[key]}</span>
    : null;

  const field = (key, label, placeholder, full = false) => (
    <div style={full ? S.formGroupFull : S.formGroup}>
      <label style={S.label}>{label}</label>
      <input style={inputStyle(key)} placeholder={placeholder} value={data[key] || ""} onChange={(e) => onChange(key, e.target.value)} />
      {errMsg(key)}
    </div>
  );

  function handleGouvernorat(val) {
    onChange("gouvernorat", val);
    onChange("delegation", ""); // reset délégation quand on change gouvernorat
  }

  return (
    <>
      <div style={S.cardTitle}>Informations légales de l'agence</div>
      <div style={S.cardSub}>Renseignez les informations officielles de votre agence immobilière.</div>
      <div style={S.formGrid}>
        {field("nom_agence", "Raison sociale *", "Ex : Agence Al Amal Immobilier", true)}
        {field("responsable", "Nom du responsable / gérant *", "Prénom Nom")}
        ...
        <div style={S.formGroup}>
          ...
          <select style={selectStyle("gouvernorat")} value={data.gouvernorat || ""} onChange={(e) => handleGouvernorat(e.target.value)}>
        ...
        <div style={S.formGroup}>
          ...
          <select
            style={{ ...selectStyle("delegation"), background: !data.gouvernorat ? "#f1f5f9" : "#fff", color: !data.gouvernorat ? "#94a3b8" : "#1e293b" }}
            value={data.delegation || ""}
            disabled={!data.gouvernorat}
            onChange={(e) => onChange("delegation", e.target.value)}
          >
            <option value="">{data.gouvernorat ? "Sélectionner..." : "— Choisir d'abord un gouvernorat —"}</option>
        ...
        {field("email", "Email professionnel *", "agence@exemple.tn")}
...
function StepDocuments({ docs, onChange, onPreview }) {
  return (
    <>
      <div style={S.cardTitle}>Documents justificatifs</div>
      <div style={S.cardSub}>Importez les documents officiels requis pour la vérification de votre agence. Formats acceptés : PDF, JPG, PNG.</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        {DOCS_CONFIG.map((cfg) => (
          <UploadField key={cfg.key} config={cfg} value={docs[cfg.key] || null} onChange={onChange} onPreview={onPreview} />
        ))}
      </div>
      <div style={{ marginTop: 20, padding: "14px 18px", background: "#fef3c7", borderRadius: 10, fontSize: 12.5, color: "#92400e", display: "flex", gap: 8 }}>
        ...
        <span>Tous les documents doivent être lisibles et en cours de validité. Des documents flous ou expirés entraîneront un rejet de votre dossier.</span>
      </div>
    </>
  );
}

function StepConvention({ accepted, onAccept, signedAt }) {
  ...
  return (
    <>
      ...
      <div ref={scrollRef} style={S.conventionBox} onScroll={handleScroll}>
        {CONVENTION_TEXT.split("\n").map((line, i) => (
          <p key={i} style={{ margin: "2px 0", whiteSpace: "pre-wrap" }}>{line || " "}</p>
        ))}
      </div>
      {!scrolled && (
      ...
      <div style={{ ...S.checkRow, opacity: scrolled ? 1 : .45, pointerEvents: scrolled ? "auto" : "none" }}>
        <input
          type="checkbox" id="accept-conv" checked={accepted} onChange={(e) => onAccept(e.target.checked)}
          style={{ width: 18, height: 18, marginTop: 2, cursor: "pointer", flexShrink: 0 }}
        />
        <label htmlFor="accept-conv" style={{ fontSize: 13.5, lineHeight: 1.5, cursor: "pointer" }}>
      ...
      {accepted && signedAt && (
        <div style={{ marginTop: 12, fontSize: 12, color: "#22c55e", fontWeight: 600 }}>
          ✓ Signé électroniquement le {new Date(signedAt).toLocaleString("fr-TN")}
        </div>
      )}
    </>
  );
...
function StepAbonnement({ selected, onSelect }) {
  ...
  return (
    <>
      ...
      <div style={S.cardSub}>Sélectionnez le plan le mieux adapté à votre activité. Vous pourrez le modifier ultérieurement.</div>
      <div style={S.plansGrid}>
        {PLANS.map((p) => {
          const isSel = selected === p.id;
          return (
            <div key={p.id} style={{ ...S.planCard(isSel, p.color), padding: "20px 16px" }} onClick={() => onSelect(p.id)}>
              {p.badge && <div style={S.planBadge(p.color)}>{p.badge}</div>}
              {p.popular && !p.badge && <div style={S.planBadge(p.color)}>Recommandé</div>}
              ...
              {p.features.map((f) => (
                <div key={f.label} style={{ ...S.planFeature, marginBottom: 6 }}>
                  ...
                  <span style={{ color: f.ok ? "#1e293b" : "#94a3b8", fontSize: 11.5 }}>
      ...
      {selected && plan && (
        <div style={{ marginTop: 24, padding: "18px 22px", background: "#f8fafc", borderRadius: 14, border: "1.5px solid #e2e8f0" }}>
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 14, color: "#1e293b" }}>Récapitulatif de commande</div>
          {plan.price === 0 ? (
            <div style={{ fontSize: 13, color: "#64748b" }}>Plan <strong>{plan.name}</strong> — aucun paiement requis.</div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                <span>{plan.name}</span><span>{plan.price} TND/mois</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#64748b" }}>
                <span>TVA (19%)</span><span>{Math.round(plan.price * 0.19)} TND</span>
              </div>
              <div style={{ height: 1, background: "#e2e8f0" }} />
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 15, fontWeight: 700 }}>
                <span>Total TTC</span><span style={{ color: plan.color }}>{Math.round(plan.price * 1.19)} TND/mois</span>
              </div>
...
function StepConfirmation({ data, docs, selectedPlan, signedAt }) {
  ...
  return (
    <>
      ...
      <div style={S.cardSub}>Votre demande d'adhésion a été enregistrée. Voici le récapitulatif et le suivi de votre dossier.</div>

      {/* Status timeline */}
      <div style={S.statusCard("#22c55e", "#f0fdf4")}>
        <CheckCircle size={22} color="#22c55e" style={{ flexShrink: 0, marginTop: 2 }} />
        <div>
          <div style={{ fontWeight: 700, fontSize: 14, color: "#15803d" }}>Dossier reçu</div>
          <div style={{ fontSize: 12.5, color: "#166534", marginTop: 2 }}>
            Votre dossier a bien été soumis le {new Date().toLocaleDateString("fr-TN")}.
          </div>
        </div>
      </div>
      <div style={S.statusCard("#f59e0b", "#fffbeb")}>
        <Clock size={22} color="#f59e0b" style={{ flexShrink: 0, marginTop: 2 }} />
        <div>
          <div style={{ fontWeight: 700, fontSize: 14, color: "#92400e" }}>Vérification en cours</div>
          <div style={{ fontSize: 12.5, color: "#78350f", marginTop: 2 }}>
            Notre équipe vérifie vos documents et informations. Délai estimé : 24 à 48h ouvrées.
          </div>
        </div>
      </div>
      <div style={{ ...S.statusCard("#94a3b8", "#f8fafc"), opacity: .6 }}>
        <CreditCard size={22} color="#94a3b8" style={{ flexShrink: 0, marginTop: 2 }} />
        <div>
          <div style={{ fontWeight: 700, fontSize: 14, color: "#475569" }}>Activation du compte</div>
          <div style={{ fontSize: 12.5, color: "#64748b", marginTop: 2 }}>
            Après validation, vous recevrez un lien de paiement par email pour activer votre abonnement {plan?.name}.
          </div>
        </div>
      </div>
      <div style={{ ...S.statusCard("#94a3b8", "#f8fafc"), opacity: .6 }}>
        <Shield size={22} color="#94a3b8" style={{ flexShrink: 0, marginTop: 2 }} />
        <div>
          <div style={{ fontWeight: 700, fontSize: 14, color: "#475569" }}>Compte actif</div>
          <div style={{ fontSize: 12.5, color: "#64748b", marginTop: 2 }}>
            Votre espace agence sera pleinement opérationnel après paiement.
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop: 28, background: "#f8fafc", borderRadius: 14, border: "1.5px solid #e2e8f0", padding: "20px 24px" }}>
        <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 14 }}>Récapitulatif du dossier</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 24px", fontSize: 13 }}>
          <div><span style={{ color: "#64748b" }}>Agence : </span><strong>{data.nom_agence || "—"}</strong></div>
          <div><span style={{ color: "#64748b" }}>Responsable : </span><strong>{data.responsable || "—"}</strong></div>
          <div><span style={{ color: "#64748b" }}>Email : </span><strong>{data.email || "—"}</strong></div>
          <div><span style={{ color: "#64748b" }}>Tél : </span><strong>{data.telephone || "—"}</strong></div>
          ...
          <div><span style={{ color: "#64748b" }}>Date signature : </span><strong>{signedAt ? new Date(signedAt).toLocaleString("fr-TN") : "—"}</strong></div>
        </div>
      </div>

      <div style={{ marginTop: 20, padding: "14px 18px", background: "#eff6ff", borderRadius: 10, fontSize: 13, color: "#1d4ed8", fontWeight: 500 }}>
        📧 Un email de confirmation a été envoyé à <strong>{data.email}</strong> avec votre numéro de dossier.
      </div>
    </>
  );
}
...
/* ─── Main component ─────────────────────────────────────────────── */
function getOnboardingKeyAgence() {
  try { const u = JSON.parse(localStorage.getItem("user")||"{}"); return `localizi_onboarding_agence_${u.id||u.username||"anon"}`; }
  catch { return "localizi_onboarding_agence_anon"; }
}

function saveOnboardingProgress(step, onProgressChange, formState = {}) {
  const status = step >= 4 ? "soumis" : "en_cours";
  const existing = (() => { try { return JSON.parse(localStorage.getItem(getOnboardingKeyAgence())||"{}"); } catch { return {}; } })();
  localStorage.setItem(getOnboardingKeyAgence(), JSON.stringify({ ...existing, step, total: 5, status, ...formState }));
  onProgressChange?.();
}

export default function AgenceOnboarding({ embedded = false, onProgressChange }) {
  const [step, setStep] = useState(() => {
    try { const s = JSON.parse(localStorage.getItem(getOnboardingKeyAgence())||"{}"); return Math.min(s.step||0, 4); } catch { return 0; }
  });
  const [legalData, setLegalData] = useState(() => {
    try { const s = JSON.parse(localStorage.getItem(getOnboardingKeyAgence())||"{}"); return s.legalData || {}; } catch { return {}; }
  });
  const [docs, setDocs] = useState({});
  const [conventionAccepted, setConventionAccepted] = useState(() => {
    try { const s = JSON.parse(localStorage.getItem(getOnboardingKeyAgence())||"{}"); return !!s.conventionAccepted; } catch { return false; }
  });
  const [signedAt, setSignedAt] = useState(() => {
    try { const s = JSON.parse(localStorage.getItem(getOnboardingKeyAgence())||"{}"); return s.signedAt || null; } catch { return null; }
  });
  const [selectedPlan, setSelectedPlan] = useState(() => {
    try { const s = JSON.parse(localStorage.getItem(getOnboardingKeyAgence())||"{}"); return s.selectedPlan || "start"; } catch { return "start"; }
  });
  ...
  function updateLegal(key, val) {
    setLegalData(d => ({ ...d, [key]: val }));
    setShowErrors(false);
  }
  function updateDoc(key, file) { setDocs(d => ({ ...d, [key]: file })); }

  function handleAcceptConvention(val) {
    setConventionAccepted(val);
    if (val && !signedAt) setSignedAt(new Date().toISOString());
    if (!val) setSignedAt(null);
  }
    ...
    try {
      ...
      for (const cfg of DOCS_CONFIG) {
        ...
        if (!file || typeof file === "string") {
          if (typeof file === "string") docsUrls[cfg.key] = file; // URL déjà uploadée
          continue;
        }
        try {
  ...
  function handleNext() {
    if (isStepValid()) {
      ...
      const next = step + 1;
      setStep(next);
      saveOnboardingProgress(next, onProgressChange, { legalData, conventionAccepted, signedAt, selectedPlan });
  ...
  const wizardContent = (
    <div style={S.container}>
      ...
      <div style={S.stepper}>
        {STEPS.map((s, i) => (
          <React.Fragment key={i}>
            ...
            <div style={{ ...S.stepItem(step === i, i < step), cursor: "pointer" }} onClick={() => { setShowErrors(false); setStep(i); saveOnboardingProgress(i, onProgressChange, { legalData, conventionAccepted, signedAt, selectedPlan }); }}>
              <div style={S.stepCircle(step === i, i < step)}>
                {i < step ? <CheckCircle size={18} /> : i + 1}
              </div>
              <div style={S.stepLabel(step === i, i < step)}>{s.label}</div>
            </div>
          </React.Fragment>
        ))}
      </div>
      ...
      {/* Card */}
      <div style={S.card}>
        {step === 0 && <StepLegal data={legalData} onChange={updateLegal} showErrors={showErrors} />}
        {step === 1 && <StepDocuments docs={docs} onChange={updateDoc} onPreview={setPreviewFile} />}
        {step === 2 && <StepConvention accepted={conventionAccepted} onAccept={handleAcceptConvention} signedAt={signedAt} />}
        {step === 3 && <StepAbonnement selected={selectedPlan} onSelect={setSelectedPlan} />}
        {step === 4 && <StepConfirmation data={legalData} docs={docs} selectedPlan={selectedPlan} signedAt={signedAt} />}

        {showErrors && step === 0 && errorCount > 0 && (
          <div style={{ marginTop: 20, padding: "12px 16px", background: "#fef2f2", border: "1.5px solid #fecaca", borderRadius: 10, display: "flex", gap: 10, alignItems: "flex-start" }}>
            <AlertCircle size={16} color="#ef4444" style={{ flexShrink: 0, marginTop: 1 }} />
            <div style={{ fontSize: 13, color: "#b91c1c" }}>
              <strong>{errorCount} champ{errorCount > 1 ? "s" : ""} obligatoire{errorCount > 1 ? "s" : ""}</strong> {errorCount > 1 ? "sont" : "est"} manquant{errorCount > 1 ? "s" : ""} ou invalide{errorCount > 1 ? "s" : ""}. Veuillez les compléter avant de continuer.
            </div>
          </div>
        )}
        {showErrors && step === 1 && DOCS_CONFIG.filter(d => docs[d.key]).length < 3 && (
          <div style={{ marginTop: 20, padding: "12px 16px", background: "#fef2f2", border: "1.5px solid #fecaca", borderRadius: 10, display: "flex", gap: 10 }}>
            <AlertCircle size={16} color="#ef4444" style={{ flexShrink: 0, marginTop: 1 }} />
            <span style={{ fontSize: 13, color: "#b91c1c" }}>Veuillez fournir au moins 3 documents pour continuer.</span>
          </div>
        )}
        {showErrors && step === 2 && !conventionAccepted && (
          <div style={{ marginTop: 20, padding: "12px 16px", background: "#fef2f2", border: "1.5px solid #fecaca", borderRadius: 10, display: "flex", gap: 10 }}>
            <AlertCircle size={16} color="#ef4444" style={{ flexShrink: 0, marginTop: 1 }} />
            <span style={{ fontSize: 13, color: "#b91c1c" }}>Vous devez lire et accepter la convention pour continuer.</span>
          </div>
        )}
        ...
        {step < 4 && (
          <div style={S.btnRow}>
            {step > 0 && (
              <button style={S.btnSecondary} onClick={() => { setShowErrors(false); const prev=step-1; setStep(prev); saveOnboardingProgress(prev, onProgressChange, { legalData, conventionAccepted, signedAt, selectedPlan }); }}>
        ...
        {step === 4 && (
          <div style={S.btnRow}>
            <button style={S.btnPrimary} onClick={() => window.location.href = "/compte?tab=annonces"}>
              Accéder à mes annonces <ChevronRight size={16} />
            </button>
          </div>
        )}
      </div>
      ...
      {step < 4 && (
        <div style={{ textAlign: "center", fontSize: 12.5, color: "#94a3b8", marginTop: 8 }}>
          Besoin d'aide ? Contactez-nous à <a href="mailto:localizi.tn@gmail.com" style={{ color: "#2563eb" }}>localizi.tn@gmail.com</a>
        </div>
      )}
        ...
        <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", padding:"0 40px", background:"linear-gradient(90deg,rgba(15,23,42,.7) 0%,rgba(15,23,42,.15) 100%)" }}>
          <div>
            <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:8 }}>
              <div style={{ width:56, height:56, borderRadius:"50%", background:"rgba(37,99,235,.85)", border:"3px solid rgba(255,255,255,.3)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                <Building2 size={26} color="#fff"/>
              </div>
              <div>
                <h1 style={{ color:"#fff", fontSize:22, fontWeight:800, margin:0, lineHeight:1.2 }}>Convention Agence</h1>
                <p style={{ color:"rgba(255,255,255,.6)", fontSize:13, margin:"4px 0 0" }}>Complétez votre dossier d'adhésion pour activer votre espace professionnel</p>
              </div>
            </div>
            <span style={{ display:"inline-block", background:"rgba(37,99,235,.8)", color:"#fff", fontSize:11.5, fontWeight:700, padding:"3px 14px", borderRadius:999, backdropFilter:"blur(4px)", marginTop:4 }}>
              Agence immobilière
            </span>
          </div>
        </div>
